...
- 运营看板返回索引任务总数、索引失败率、索引积压压力和运营待办合计，前端总览页可直接展示压力指标
...
- 知识库列表支持按名称或说明搜索，前端搜索框请求携带 `keyword` 并同步选择器结果
- 普通用户访问用户管理接口返回 403
- 普通用户访问管理员接口返回 403
...
- Dockerfile 和 Compose 部署契约有静态测试守护，防止构建阶段、MySQL 服务、专用应用账号、应用环境变量和 volume 声明被误删
...
31. 运营看板按现有只读计数派生索引失败率、积压压力和运营待办合计，前端展示百分比和合计数
32. 文档列表带 `page/size` 时返回分页结构，搜索和状态筛选在服务端生效，非法分页参数返回 `VALIDATION_ERROR`
33. 管理端用户、token 会话和审计列表带 `page/size` 时返回分页结构，非法分页参数返回 `VALIDATION_ERROR`
34. 相似工单检索根据问题关键词命中相关历史工单，接口返回 score 大于 0 的前 5 条，并包含 `matchedKeywords` 与 `matchSummary`
35. MySQL 迁移脚本覆盖全部 JPA 实体表和持久化列，`MysqlSchemaEntityAlignmentTest` 不连接 MySQL 也能发现漏改迁移
36. 前端普通用户只能看到允许的路由入口，直接访问无权限路由会回到可见的默认页
37. 前端路由切换后刷新页面仍保持当前业务页，浏览器前进后退同步当前导航高亮
38. 前端审计页按 traceId 和事件过滤时，请求包含过滤参数并返回对应时间线，traceId 可复制
39. AI 运行配置接口返回 chat/embedding 状态、readinessLevel 和 warnings，响应体不包含 API key 明文
40. 前端 AI 配置页只展示凭证是否已配置，管理员可刷新运行配置，普通用户看不到入口
41. 前端相似工单列表展示命中关键词和相似原因，帮助用户判断是否复用历史工单经验
42. 前端回答引用和提交后的工单号可复制，并通过通知反馈复制结果
43. 知识库搜索按名称或说明过滤，未命中时选择器清空并提示当前显示 0 个知识库
...
- Dockerfile 能声明前端构建和前后端一体化 Jar 打包阶段，Compose 配置能声明 MySQL 业务库、专用应用账号、应用环境变量、健康检查和持久化 volume
- 没有已知的高优先级阻塞问题
