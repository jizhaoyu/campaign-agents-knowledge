...
- 2026-06-06：文档管理、审批队列、用户、Token 会话和审计列表增加首次查询骨架屏，查询中不再误展示空列表
- 2026-06-06：问答生成、工单草稿、工单提交和相似工单查询增加加载态与单飞保护，慢请求期间给出明确反馈
- 2026-06-06：文档上传、重建索引、批量重试失败文档和删除文档增加加载态与单飞保护，避免慢请求期间重复提交
- 2026-06-06：审批决策、用户解锁、Token 会话吊销和用户会话批量吊销增加加载态与单飞保护，避免管理端副作用操作重复提交
...
- 当前已有 Playwright 核心 E2E，并覆盖复制成功/剪贴板不可用降级、运营/AI 首屏骨架屏、文档/审批/管理列表首次查询骨架屏、文档写操作加载态单飞保护、管理端副作用操作加载态单飞保护、问答/工单交互加载态单飞保护、运营/AI 刷新加载态单飞保护、运营/AI 加载失败原位重试恢复、文档上传校验失败、审批业务冲突、AI 配置读取后端异常、AI 配置网络不可达、知识问答网络不可达和工单提交网络不可达；后续应补单元测试和更多边界失败场景 E2E
- 当前 Vite 代理默认后端端口为 `8080`，已支持通过 `VITE_API_TARGET` 覆盖，前端环境变量样例已放在 `frontend/.env.example`
